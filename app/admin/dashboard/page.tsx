...
export default function AdminOverviewPage() {
  ...
  return (
    <div className="space-y-10">
      <header>
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-smoke">Admin overview</p>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4" aria-label="Studio statistics">
        {stats.map((stat) => (
          <div key={stat.label} className="admin-panel group p-5 transition-colors hover:border-bone/20 md:p-6">
            ...
            <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.1em] text-smoke">{stat.label}</p>
          </div>
        ))}
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="admin-panel">
          <div className="flex items-center justify-between border-b border-bone/10 px-5 py-4 md:px-6">
            ...
            <Link href="/admin/dashboard/projects" className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.1em] text-smoke hover:text-bone">
        ...
        <section className="admin-panel">
          <div className="flex items-center justify-between border-b border-bone/10 px-5 py-4 md:px-6">
            ...
            <Link href="/admin/dashboard/inquiries" className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.1em] text-smoke hover:text-bone">
              View all <ArrowUpRight size={12} />
            </Link>
          </div>
          <div className="divide-y divide-bone/10">
            {inquiries.slice(0, 6).map((inquiry) => (
              <div key={inquiry.id} className="px-5 py-4 md:px-6">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <p className="shrink-0 font-mono text-[10px] text-smoke">{new Date(inquiry.createdAt).toLocaleDateString("en-IN")}</p>
                </div>
                <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.06em] text-smoke">
